...
public class ImageProcess extends CordovaPlugin implements EditImgInterface {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        mSavedFilePath = args.getString(0);
        if (TextUtils.isEmpty(mSavedFilePath)) {
            mSavedFilePath = FileUtils.createFile(activity);
        }
        if (action.equals(METHOD_OPEN_CAMERA)) {
            openCamera();
        } else if (action.equals(METHOD_OPEN_ALBUM)) {
            openAlbum();
        } else if (action.equals(METHOD_OPEN_CROP)) {
            String selectImagePath = args.getString(1);
    ...
    public void openCrop(Uri selectedImageUri) {
        if (selectedImageUri.getScheme().equals("file")) {//判断uri地址是以什么开头的
            selectedImageUri = URIUtils.getFileUri(activity, selectedImageUri);
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        ToastUtils.toastMessage(activity, "onActivityResult --- " + requestCode + " --- " + resultCode);
    ...
    public void onEditImgResult(int code, EditImageMessage editImageMessage) {
        ...
        ToastUtils.toastMessage(activity, "code = " + code + " editImageMessage - " + editImageMessage.toString());
    }
}